...
people.forEach((human)=>{
    ...
    newRow.insertAdjacentHTML('afterbegin', `<td>${human.name}</td> <td>${human.sex === 'm' ? 'Male' : 'Female'}</td> <td>${human.born}</td> <td>${human.died}</td> <td>${human.died - human.born}</td> <td>${Math.ceil(human.died / 100)}</td>`);
